...
#####Data Import - Obtained from cBioPortal#####
...
######TFEB and genesets######
geneset <- c("TFEB", "TFE3", "MITF", "FLCN", "PPP3CA", "PPP3CB", "PPP3CC", "PPP3R1", "PPP3R2", 
             "RRAGC", "RRAGD", "FNIP1", "FNIP2", "MAP4K3", "STUB1", "	PPP2CA", "PPP2CB")
...
######TCGA Data - obtained from Firehose########
...
tcga.hrd <- read_csv("tcga_brca_hrd.csv")
...
#####Remove matched normal samples where applicable ######
#sorts by names then for the duplicated samples, selects the first one in list, which will be the tumor sample
tcgabc.u <- tcgabc.b[order(tcgabc.b$sample, decreasing = F),]
...
tcgabc.clin <- dplyr::inner_join(x = tcgabc.clin, y = tcga.hrd, by = c("sample" = "TCGA Participant Barcode"))
...
####### Create plots for all genes in TFEB geneset #######

ihc_er <- tcgasample.2 %>% select(patient_id, ihc_er)
ihc_er$ihc_er <- factor(ihc_er$ihc_er, levels = c("Positive","Negative"))
ihc_er$ihc_er <- recode_factor(ihc_er$ihc_er, Positive = "ER+", Negative = "ER-")

tfeb.er <- dplyr::inner_join(x=tcgabc.c, y=ihc_er, by = c("sample" = "patient_id"))
write.csv(tfeb.er, "tcga_tfebgeneset_ihcer.csv")

tfeb.melt <- melt(tfeb.er)

a <- ggplot(tfeb.melt, aes(value, fill = ihc_er))
...
c <- tfeb.melt %>% drop_na(ihc_er) %>% ggplot(aes(ihc_er, value)) 
c + geom_jitter(alpha = 0.2, colour = "green4", size = 1, width = 0.2)+
  geom_boxplot(fill = "grey65", colour = "black",  
               outlier.shape = NA, alpha=0.75, 
               fatten = 1, lwd = 0.5, width = 0.5, notch = TRUE)+
  ...
  facet_wrap(vars(variable), scales = "free") +
  theme_classic()+
  labs(title = "", y="Normalized Expression", x=NULL)+
  theme(text = element_text(size=14),
        legend.title = element_text(size = 12),
        axis.text = element_text(size=9),
        plot.title = element_text(hjust = 0.5),
